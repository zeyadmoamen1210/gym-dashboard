  ...
  <div class="all-receptionests">
      <div class="container">
          <div class="head-section">
              <div>
                  <StarHeader title="Trainees List" />
              </div>
                <div class="search">
                    <div class="form-group" style="display:flex">
                        ...
                        style="padding: 4px 3px;border-radius: 7px;"
                            transparent
                            :active="active == 1"
                            @click="$router.push(`/add-participant`)"
                        >
...
export default {
    ...
    watch:{
        page(val){
            this.getTrainee(val, this.search);
        }
    },
    methods:{
        updatePass(){
        ...
        pushToEdit(recep){
            ...
            this.$router.push(`/edit-trainee/${recep.id}`);
        },
        searchInTrinee(){
            this.getTrainee(this.page, this.search);
        ...
        getTrainee(page = 1, username= ''){
            const loading = this.$vs.loading();
            axiosApi.get(`/getUsers?role=trainee&page=${page}&username=${username}`).then(res => {
